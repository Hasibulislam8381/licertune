@extends('backend.app')
@section('title', 'Edit Software Fee')
@section('page-content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <div class="p-3 card">
                    <form action="{{ route('backend.software_fees.update', $data->id) }}" method="POST"
                                ...
                                @foreach ($languages as $lang)
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea id="summernote" name="description_{{ $lang }}" class="form-control" rows="3">{{ $translations[$lang]['description'] ?? '' }}</textarea>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
